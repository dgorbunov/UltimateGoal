...
public class Camera implements Controller {
    ...
    public Camera(HardwareMap hwMap, Telemetry tel) {
        hardwareMap = hwMap;
        telemetry = tel;
    }
...
//    }
    ...
    public Integer ringsToInt(String element){
        if (element.equals(Camera.LABEL_NO_ELEMENT)) return INT_NO_ELEMENT;
        else if (element.equals(Camera.LABEL_FIRST_ELEMENT)) return INT_FIRST_ELEMENT;
        else if (element.equals(Camera.LABEL_SECOND_ELEMENT)) return INT_SECOND_ELEMENT;
        else {
    ...
    private void initTfod() {
        ...
        if (Auto.getSide() != null) {
            if (Auto.getSide() == FieldConstants.LeftSide) tfod.setClippingMargins(100, 215, 350, 0);
